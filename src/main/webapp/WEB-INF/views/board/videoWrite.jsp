...
<form action="/board/videoWrite" method="post" enctype="multipart/form-data" id="form_write">
...
<script>
	$('#test').click(function(){
		if($('#file').val().length > 0) {
		       ...
		       let max_fileSize = 104857600;  // 파일 제한 10MB
		       var check_file_type=new Array();
		       check_file_type=['mp4','mov','avi','wmv','mpeg'];  //허용 확장자

		       //파일 확장자 체크
			   if(check_file_type.indexOf(fileType) == -1) {
		             alert('mp4, mov, avi, wmv, mpeg 확장자의 파일만 업로드할 수 있습니다.');
			         return false;
			   }
			   //파일 사이즈 체크
			   if( max_fileSize < $("#file")[0].files[0].size ) {
			         alert('파일 용량은 100MB 이하로 제한됩니다.');
			         return false;
			   }
